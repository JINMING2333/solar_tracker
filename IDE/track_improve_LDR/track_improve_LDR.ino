...
// —— LDR 微调参数 ——
// 若接法是“越亮 ADC 值越小”（常见分压接法），保持为 true；否则改为 false
const bool   LDR_LOW_IS_BRIGHT = true;
const uint8_t LDR_SAMPLES      = 4;     // 取多次平均，去抖
const int     LDR_DIFF_THRESH  = 35;    // 亮度差阈值（可调）
const float   LDR_STEP_DEG     = 2.0;   // 每次微调的角度（1~3° 之间看机构间隙）
const uint8_t LDR_MAX_STEPS    = 25;    // 单次微调最多步数，避免来回抖动
const uint16_t LDR_SETTLE_MS   = 120;   // 每步后等待传感器稳定
...
// 返回：差值的绝对值；并通过引用给出“往西/往东”的建议方向
// 规则：若 LDR_LOW_IS_BRIGHT=true，把两路转成亮度 B=1023-ADC，再比较
int ldrDeltaSuggest(bool &goWest /*true 往西，false 往东*/, int &eRaw, int &wRaw) {
  eRaw = readLdrAvg(LDR_EAST_PIN);
  wRaw = readLdrAvg(LDR_WEST_PIN);
  int eB, wB;  // brightness
  if (LDR_LOW_IS_BRIGHT) {
    eB = 1023 - eRaw;
    wB = 1023 - wRaw;
  } else {
    eB = eRaw;
  ...
  // goWest=true 表示“西侧更亮，需要向西转”
  goWest = (wB > eB);
  return abs(wB - eB);
}

// 到达天文角后用 LDR 微调，直到差值小于阈值或触步上限
bool fineTuneByLDR() {
  float cur = getAngle();
  for (uint8_t step = 0; step < LDR_MAX_STEPS; step++) {
    bool goWest; int eRaw, wRaw;
    int diff = ldrDeltaSuggest(goWest, eRaw, wRaw);

    Serial.print("LDR E="); Serial.print(eRaw);
    Serial.print(" W=");     Serial.print(wRaw);
    Serial.print(" |diff|=");Serial.print(diff);
    Serial.print(" -> ");    Serial.println(goWest ? "WEST" : "EAST");

    if (diff <= LDR_DIFF_THRESH) return true;

    float next = cur + (goWest ? +LDR_STEP_DEG : -LDR_STEP_DEG);
    next = constrain(next, -EAST_LIMIT, WEST_LIMIT);

    if (!moveToAngle(next, 3000)) return false;
    delay(LDR_SETTLE_MS); // 让光敏、电机、编码器稳定
    cur = getAngle();
  }
  return false; // 步数用尽仍未达阈值
}
...
void loop() {
  ...
  if (fabs(tgt - curAngle) >= ROTATE_THRESHOLD) {
    ...
    if (!ok) {
      Serial.println("⚠️ Move failed!");
    } else {
      // ✅ 到达天文目标角后，用 LDR 做精调（仅在白天）
      if (sunEl > 5.0) {
        bool tuned = fineTuneByLDR();
        if (!tuned) Serial.println("⚠️ LDR tuning not converged");
      }
    }
  }else {

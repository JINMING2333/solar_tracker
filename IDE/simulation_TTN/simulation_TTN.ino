...
float mockAz = -90;
const float mockStep = 1.5;
const uint32_t STEP_INTERVAL = 10000;
uint32_t lastStep = 0;
...
bool moveToAngle(float tgtDeg, uint32_t timeout = 8000) {
  ...
  while (fwd ? encCount < tgtPulse : encCount > tgtPulse) {
    if (millis() - lastPrint > 50) {
      ...
      energyActive_mWh += power * 0.05 / 3600.0;
      if (current > 0) {
...
void loop() {
  ...
  float tgt = constrain(mockAz, -EAST_LIMIT, WEST_LIMIT);
  ...
  if (now - lastStep > STEP_INTERVAL) {
    // Serial.print("⚡ 运行能耗: "); Serial.print(energyActive_mWh, 3);
    // Serial.print(" mWh，静止能耗: "); Serial.print(energyIdle_mWh, 3);
    // Serial.print(" mWh，总能耗: "); Serial.print(energyActive_mWh + energyIdle_mWh, 3);
    // Serial.print(" mWh，逻辑电路: "); Serial.print(energyLogic_mWh, 3);
    // Serial.print(" mWh，太阳吸收: "); Serial.print(energySolar_mWh, 3); Serial.println(" mWh");
    mockAz += mockStep;
    if (mockAz > 90) {
